-- v21.3 인사카드 DB/RLS 복구 SQL
-- 기존 직원/급여 데이터는 삭제하지 않습니다.

alter table public.employees add column if not exists hire_date date;
alter table public.employees add column if not exists retire_date date;
alter table public.employees add column if not exists employment_status text default '재직';
...
COLUMN_KR = {
    ...
    "hire_date":"입사일","retire_date":"퇴사일","employment_status":"재직상태","phone":"연락처","email":"이메일","address":"주소",
...
if active_tab == 1:
    ...
    with tab_card:
        ...
        def _datev(k, fallback=None):
            """Streamlit date_input용 안전한 날짜값."""
            raw=_v(k, None)
            if raw is None or raw == "" or str(raw).lower() in ("none","nan","nat"):
                return fallback
            try:
                dt=pd.to_datetime(raw,errors="coerce")
                return fallback if pd.isna(dt) else dt.date()
            except Exception:
                return fallback
                ...
                with st.form("v21_history_form",clear_on_submit=True):
                    ...
                    if st.form_submit_button("인사이력 등록"):
                        try:
                            ...
                            _msg=str(e).lower()
                            if "does not exist" in _msg or "schema cache" in _msg:
                                st.error("인사이력 저장 실패: employee_history 테이블이 없거나 스키마 반영이 필요합니다.")
            ...
            with st.form("v21_secure_form"):
                ...
                if st.form_submit_button("🔐 민감정보 저장"):
                    ...
                    try:
                        ...
                        _msg=str(e).lower()
                        if "does not exist" in _msg or "schema cache" in _msg:
                            st.error("저장 실패: employee_private_info 테이블이 없거나 스키마 반영이 필요합니다.")
                        elif "row-level security" in _msg or "permission" in _msg or "42501" in _msg:
...
create index if not exists idx_employee_history_emp_date
on public.employee_history(emp_id,effective_date desc);
...
-- 이전/중복 정책 정리
drop policy if exists employee_history_select_v21 on public.employee_history;
drop policy if exists employee_history_write_v21 on public.employee_history;
drop policy if exists employee_private_admin_select_v21 on public.employee_private_info;
drop policy if exists employee_private_admin_write_v21 on public.employee_private_info;
drop policy if exists employee_history_select_v213 on public.employee_history;
drop policy if exists employee_history_insert_v213 on public.employee_history;
drop policy if exists employee_history_update_v213 on public.employee_history;
drop policy if exists employee_history_delete_v213 on public.employee_history;
drop policy if exists employee_private_select_v213 on public.employee_private_info;
drop policy if exists employee_private_insert_v213 on public.employee_private_info;
drop policy if exists employee_private_update_v213 on public.employee_private_info;
drop policy if exists employee_private_delete_v213 on public.employee_private_info;

-- JWT metadata를 직접 읽어 helper 함수 의존성 제거
create policy employee_history_select_v213 on public.employee_history
for select to authenticated using (
 coalesce(auth.jwt()->'user_metadata'->>'role','') in ('admin','manager')
 or emp_id = coalesce(auth.jwt()->'user_metadata'->>'emp_id','')
);
create policy employee_history_insert_v213 on public.employee_history
for insert to authenticated with check (
 coalesce(auth.jwt()->'user_metadata'->>'role','') in ('admin','manager')
);
create policy employee_history_update_v213 on public.employee_history
for update to authenticated using (
 coalesce(auth.jwt()->'user_metadata'->>'role','') in ('admin','manager')
) with check (
 coalesce(auth.jwt()->'user_metadata'->>'role','') in ('admin','manager')
);
create policy employee_history_delete_v213 on public.employee_history
for delete to authenticated using (
 coalesce(auth.jwt()->'user_metadata'->>'role','') in ('admin','manager')
);

create policy employee_private_select_v213 on public.employee_private_info
for select to authenticated using (
 coalesce(auth.jwt()->'user_metadata'->>'role','')='admin'
);
create policy employee_private_insert_v213 on public.employee_private_info
for insert to authenticated with check (
 coalesce(auth.jwt()->'user_metadata'->>'role','')='admin'
);
create policy employee_private_update_v213 on public.employee_private_info
for update to authenticated using (
 coalesce(auth.jwt()->'user_metadata'->>'role','')='admin'
) with check (
 coalesce(auth.jwt()->'user_metadata'->>'role','')='admin'
);
create policy employee_private_delete_v213 on public.employee_private_info
for delete to authenticated using (
 coalesce(auth.jwt()->'user_metadata'->>'role','')='admin'
);

grant select,insert,update,delete on public.employee_history to authenticated;
